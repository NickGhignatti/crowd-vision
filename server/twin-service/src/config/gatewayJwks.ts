...
// Minimal, dependency-free JWKS client: fetch, cache briefly, index by kid.
// A hand-rolled ~30 lines beats pulling in jwks-rsa (which drags in the
// ESM-only `jose`, incompatible with this service's current Jest transform
// config) for what Node's own crypto.createPublicKey already does natively.
