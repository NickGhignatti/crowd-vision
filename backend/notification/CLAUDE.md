...
Group `alerts`, `auto.offset.reset=earliest` — a breach produced while it
is down is processed on return. Payload is `telemetry_schema::AlertEvent`, parsed once in
...
what the winner cached. Failures are uncached by design, so without this a slow digital-twin
