...
publishes Kafka event, resolves `ready`/`failed` on telemetry's completion event.

`failed` (publish refused, or telemetry's own callback) deletes the twin
(`BuildingStore::delete`) and calls notification's `POST /trigger` as a system caller
(`OutboundConfig::notify_provisioning_failed`, `NOTIFICATION_URL`) — notify **before**
delete, since `/trigger` resolves the building's domains by calling back into digital-twin.
