...
impl Provisioning {
    ...
    // failed before telemetry ever heard about it, or telemetry rejected it later.
    // Notify before deleting: notification resolves the building's domains by
    // calling back into digital-twin, so the twin must still exist when it does.
...
mod tests {
    ...
    async fn provisioning_stores_the_twin_and_tells_downstream() {
        ...
        assert_eq!(
            ...
            UploadStatus::Pending,
            "a successful publish is not telemetry's outcome -- \
    ...
    async fn resolving_with_an_error_marks_the_upload_failed() {
        ...
        assert!(h.queue.errors.lock().unwrap()["b1"].contains("telemetry said no"));
        assert!(
            h.store.get("b1").is_none(),
            "a building telemetry rejects must not linger in the store"
        );
        assert_eq!(
            h.sync.failure_notifications.lock().unwrap()[0],
            ("b1".to_string(), "telemetry said no".to_string())
        );
    }
