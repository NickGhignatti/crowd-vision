...
/// The one place that wires a full digital-twin app for tests: a fresh,
...
pub async fn build(label: &str) -> TestApp {
    ...
    let db_name = format!("digital_twin_{label}_{}", Uuid::new_v4().simple());
...
fn is_test_database(name: &str) -> bool {
    let Some(suffix) = name
        .strip_prefix("digital_twin_api_")
        .or_else(|| name.strip_prefix("digital_twin_cucumber_"))
    else {
        return false;
    };
