...
the update leaving socket-service is at most 1 second, and the 99th
...
proxy for "leaves socket-service" (the test client is the very next hop)
...
socket-service if this ever needs to be tighter than this test can prove.
"""
...
from support.config import SOCKET_SERVICE_URL
...
def _one_round_trip(index: int) -> float:
    ...
    with httpx.Client(timeout=10.0) as client:
        ...
        dashboard.connect(SOCKET_SERVICE_URL, headers=headers)
        try:
