"""
Migrated from the former backend/tests/metrics.test.ts (Node/Jest). Verifies
contracts-service's metrics catalog. Not one of the LD-4 acceptance criteria
...
contracts-service at all (grepped main.rs's route table directly to
...
def metrics_catalog() -> dict:
    with httpx.Client(timeout=10.0) as client:
        response = client.get(
            CONTRACTS_SERVICE_URL, headers={"x-gateway-claims": claims_header()}
        )
