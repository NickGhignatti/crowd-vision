...
Rust / Axum / MongoDB. Conversation history + streaming proxy to `agent`.
No AI here — no LLM, no retrieval. That is all `agent`.
...
conversation. A stream that ends without `done` is `"agent returned an invalid
...
- **Citation fields stay snake_case** (`chunk_id`, `document_id`, `source`,
  `section_path`) — agent's Python payload, stored and returned as-is. A blanket
