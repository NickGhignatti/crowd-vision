#!/usr/bin/env bash
# Runs tests/*.rs against a real MongoDB, both inside the same compose
...
PROJECT_NAME="telemetry-service-integration-tests"
COMPOSE=(docker compose -p "$PROJECT_NAME" -f docker-compose.test.yml)

"${COMPOSE[@]}" up --build --exit-code-from telemetry-service-test --abort-on-container-exit
exit_code=$?
