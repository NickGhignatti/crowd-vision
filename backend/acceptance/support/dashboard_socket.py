...
class DashboardSocket:
    ...
    frontend/src/stores/sensorData.ts uses against the real socket.
    """
    ...
    def connect(self, url: str, headers: dict[str, str]) -> None:
        """Connects and blocks until socket confirms the room join.

        `subscribe_building` is acknowledged because the join is not immediate
        — the handler resolves the building's domains through digital-twin
