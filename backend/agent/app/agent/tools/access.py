...
async def get_authorized_building(
    ...
    try:
        ...
        return None, ToolResult(content="digital-twin is unavailable", is_error=True)

    if response.status_code == 404:
        return None, ToolResult(content="building unavailable or inaccessible", is_error=True)
    if response.status_code >= 400:
        return None, ToolResult(
            content=downstream_error("digital-twin", response),
            is_error=True,
        )
    ...
    try:
        ...
        return None, ToolResult(content="digital-twin returned invalid data", is_error=True)
