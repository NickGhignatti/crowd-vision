...
class GetLatestSensorDataTool:
    ...
    async def run(self, args: GetLatestSensorDataArgs, ctx: ToolContext) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(content="telemetry is unavailable", is_error=True)

        if response.status_code == 400:
            return ToolResult(content=_sensor_empty_payload(building, args.metric, key="readings"))
        if response.status_code >= 400:
            return ToolResult(
                content=downstream_error("telemetry", response),
                is_error=True,
            )
        try:
            body = response.json()
        except ValueError:
            return ToolResult(content="telemetry returned invalid data", is_error=True)
