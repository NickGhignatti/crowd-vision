...
- `list_buildings`, `get_building`, `list_rooms`, `get_room`: structural data from `digital-twin`.
...
just stack logs agent      # follow agent logs
...
| Follow service logs (**root**) | `just stack logs agent` |
...
# direct invocation (from server/agent):
...
| `DIGITAL_TWIN_URL` / `TWIN_TIMEOUT_SECONDS` | `http://digital-twin:3000` / `10` | Structural building backend and request timeout |
| `TELEMETRY_URL` / `TELEMETRY_TIMEOUT_SECONDS` | `http://telemetry:3000` / `10` | Sensor-data backend and request timeout |
...
- For real user traffic, confirm the request reaches agent through the edge (which
  injects `x-gateway-claims`) or through chat — a caller hitting `/agent` directly with a
...
Confirm `digital-twin` is running and `DIGITAL_TWIN_URL` is reachable from where the agent
runs. The Docker default `http://digital-twin:3000` does not resolve from a host process;
override it with the host-accessible digital-twin URL when debugging outside Docker.
For occupancy, temperature, air quality, or trend questions, also confirm `telemetry`
is reachable through `TELEMETRY_URL`. A `429` tool result means the downstream read
...
The current Kubernetes manifests deploy `agent` and `agent-db`, but do not deploy
Langfuse or the one-shot documentation ingester. Verify the `agent-secret`
