"""Run the golden dataset against a running agent.
...
LOCAL_HOSTS = {"localhost", "127.0.0.1", "::1", "0.0.0.0", "agent", "host.docker.internal"}
JUDGE_RUBRICS = {
...
def _eval_jwt_secret() -> str:
    ...
    if not secret:
        raise SystemExit(
            ...
            f"{REPO_ROOT / '.env'} (and on agent, so it accepts the minted token)."
        )
    return secret
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the golden dataset against agent.")
    parser.add_argument(
        "--models",
