...
#[tokio::test]
async fn the_catalog_deserialises_into_the_shape_dashboard_parses() {
    ...
    let MetricsDiscoveryResponse::ServiceContract(contract) = discovered else {
        panic!("telemetry advertises a service-shaped catalog");
    };
    assert_eq!(contract.service, "telemetry");
    assert!(
        contract
            .metrics
            .iter()
