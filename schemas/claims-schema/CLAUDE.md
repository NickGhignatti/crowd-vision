...
Go half: `backend/libs/auth-contracts` (`StandardClaims` + the `roles.json` ladder). Same wire
shape, one shared fixture — `schemas/fixtures/standard-claims.json`, asserted by
...
`chat`, `dashboard`, `notification`, `socket`,
`telemetry`, `digital-twin`. All build from **repo-root context** so the sibling path
