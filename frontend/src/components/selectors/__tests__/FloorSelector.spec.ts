  ...
  useI18n: () => ({
    t: (key: string) => {
      const translations: Record<string, string> = {
        'model.controls.floorSelection': 'FloorSelector Selection',
        'model.controls.floor': 'FloorSelector',
      }
...
describe('FloorSelector', () => {
  describe('rendering', () => {
    it('renders the label', () => {
      ...
      expect(wrapper.find('label').text()).toBe('FloorSelector Selection')
    })
    ...
    it('renders each floor with its index and height', () => {
      ...
      expect(options[0]?.text()).toBe('FloorSelector 0 (0m)')
      expect(options[1]?.text()).toBe('FloorSelector 1 (3m)')
      expect(options[2]?.text()).toBe('FloorSelector 2 (6m)')
    })
