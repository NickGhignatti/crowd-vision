  ...
  <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden transition-all duration-300">
    <div @click="toggleAccordion"
      ...
      <div class="flex items-center gap-3">
        ...
        <span class="text-[10px] font-bold px-2.5 py-1 rounded-full border shadow-sm tracking-wider"
      ...
      <div class="flex items-center gap-2">
        <button @click.stop="$emit('notification-trigger', domainGroup.name)"
          ...
          <i class="ph text-xl"
            :class="notificationStore.isSubscribed(domainGroup.name) ? 'ph-bell' : 'ph-bell-slash'"></i>
        </button>

        <UploadModelButton v-if="domainGroup.canUpload" :is-uploading="isUploading"
          @click.stop="$emit('upload', domainGroup.name)" />

        <div v-if="domainGroup.subdomains.length > 0"
          class="w-9 h-9 flex items-center justify-center rounded-full transition-transform duration-300 text-slate-400"
          :class="isOpen ? 'rotate-180 bg-slate-200 text-slate-700' : 'bg-slate-100 hover:bg-slate-200'
            ">
    ...
    <div v-show="isOpen" class="bg-slate-50 px-5 pb-5 pt-3 space-y-2">
      <SubdomainCard v-for="sub in domainGroup.subdomains" :key="sub.name" :name="sub.name"
        :display-name="sub.displayName" :parent-domain-name="domainGroup.name" :can-upload="domainGroup.canUpload"
        :is-uploading="isUploading" @select="$emit('select-domain', $event)" @upload="$emit('upload', $event)"
