...
import { buildRows, sensorKinds } from '@/utils/tableRows.ts'
...
const {
  ...
  savePreferences,
  cancelEdit,
} = useColumnManager(toRef(props, 'headers'), buildingIdRef, (updated) =>
  emit('headers-updated', updated),
)

const { readings, isLoading: loadingReadings } = useBuildingReadings(
  buildingIdRef,
  computed(() => sensorKinds(localHeaders.value)),
)

const enrichedItems = computed<TableBody[]>(() =>
  props.roomsData ? buildRows(props.roomsData, localHeaders.value, readings.value) : [],
)
  ...
  >
    ...
    <div class="flex-1 min-h-0 overflow-auto bg-white relative">
      <table class="w-full text-left border-collapse">
        ...
        <tbody v-else-if="loadingReadings">
          <StatusRecord :colspan="localHeaders.length + (isEditMode ? 1 : 0)" pulse>
            {{ t('model.loading') }}
          </StatusRecord>
        </tbody>
