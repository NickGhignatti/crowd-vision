...
function createDepthMaterial(shading: DepthShading, additive: boolean) {
  ...
  const glow = uniform(shading.edgeStrength).mul(edgeGlow(shading.edgeWidth))
...
export const depthFade: RenderStyle = {
  id: 'depth',
  material: (role, theme) => createDepthMaterial(config[role], theme === 'dark'),
}
