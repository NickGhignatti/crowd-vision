...
function createShellMaterial(
  ...
  material.opacityNode = min(
    fresnel.add(uniform(edgeStrength).mul(edgeGlow(edgeWidth, hiddenWalls))),
    1,
  )
...
export const ghostShell: RenderStyle = {
  ...
  material: (role, theme, hiddenWalls) =>
    createShellMaterial(config[role], theme === 'dark', hiddenWalls),
}
