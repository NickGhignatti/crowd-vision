...
vi.mock('@/composables/useNavLinks', () => ({
  useNavLinks: () => ({ links: [{ to: '/dashboard', label: () => 'Dashboard' }] }),
}))
...
const stubs = {
  NavLink: {
    props: ['to', 'isLoggedIn'],
    emits: ['locked-click'],
    template: '<button class="nav-link-stub" @click="$emit(\'locked-click\')"><slot /></button>',
  },
  LoginModal: LoginModalStub,
  SignUpModal: SignUpModalStub,
  ProfileDropdown: {
...
describe('NavBar.vue', () => {
  describe('auth state', () => {
    ...
    it('delegates logout to handleLogout when ProfileDropdown emits it', async () => {
      isLoggedIn.value = true
      const wrapper = mount(NavBar, { global: { stubs } })
  ...
  describe('login modal', () => {
    it('opens when a locked NavLink emits locked-click', async () => {
      const wrapper = mount(NavBar, { global: { stubs } })
      await wrapper.find('.nav-link-stub').trigger('click')
    ...
    it('closes when it emits close', async () => {
      const wrapper = mount(NavBar, { global: { stubs } })
      await wrapper.find('.nav-link-stub').trigger('click') // open first
