...
<template>
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col h-[600px]">
    ...
    <div v-if="!domain" class="flex-1 flex flex-col items-center justify-center gap-3 text-slate-400">
    ...
    <div v-else class="flex-1 flex flex-col gap-4 min-h-0">
      <div class="flex gap-2 flex-wrap">
        <button v-for="role in roles" :key="role" @click="selectedRole = role"
          class="px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all duration-150" :class="selectedRole === role
        ...
        <template v-if="selectedRole">
          ...
          <Transition name="fade" mode="out-in">
            <div :key="selectedRole" class="flex flex-col items-center gap-3">
              <img v-if="qrDataUrls[selectedRole]" :src="qrDataUrls[selectedRole]" alt="QR code"
