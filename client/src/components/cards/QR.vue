...
const ROLE_META: Record<string, { label: string; badge: string; tab: string }> = {
  admin: {
    label: 'Admin',
  ...
  standard_customer: {
    ...
    tab: 'text-slate-700 border-slate-500 bg-slate-50',
  },
}
...
<template>
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col h-[600px]">
    ...
    >
      ...
      <p class="text-sm font-medium">Select a domain to see its QR codes</p>
    </div>
        ...
        <template v-if="selectedRole">
          ...
          <Transition name="fade" mode="out-in">
            <div :key="selectedRole" class="flex flex-col items-center gap-3">
              ...
              <p class="text-xs text-slate-400 text-center">{{ t('domains.administration.scanQRCode')}}</p>
            </div>
          </Transition>
        </template>
      </div>
