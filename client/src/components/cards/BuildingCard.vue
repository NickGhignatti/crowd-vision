...
<template>
  <div>
    <div class="p-4 rounded-xl border cursor-pointer transition-all duration-200 ease-in-out relative group" :class="[
      isSelected
      ...
      <div v-if="isSelected" class="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
        ...
        <button @click.stop="emit('toggle-controls', $event)" class="p-1.5 rounded-lg transition-all" :class="showControls
    ...
    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-y-2"
      ...
      <div v-if="isSelected && showControls && buildingModel && availableFloors.length > 0" class="mt-2 ml-4 relative">
        <div
    ...
    <EditBuildingModal :is-open="isEditModalOpen" :building="buildingModel" @close="isEditModalOpen = false"
